...
PartWidget::PartWidget(const Document *document, dust3d::Uuid partId) :
    m_document(document),
    m_partId(partId)
{
    ...
    previewAndToolsLayout->setContentsMargins(2, 0, 0, 0);
...
void PartWidget::showColorSettingPopup(const QPoint &pos)
{
    ...
    if (m_document->materialIdList.empty()) {
        ...
        materialListWidget->setHasContextMenu(false);
        materialListWidget->selectMaterial(part->materialId);
        connect(materialListWidget, &MaterialListWidget::currentSelectedMaterialChanged, this, [=](dust3d::Uuid materialId) {
...
void PartWidget::showCutRotationSettingPopup(const QPoint &pos)
{
    ...
    if (part->hasCutFaceFunction()) {
        ...
        for (size_t i = 0; i < cutFaceList.size(); ++i) {
            if (dust3d::CutFace::UserDefined == part->cutFace) {
                if (QString(part->cutFaceLinkedId.toString().c_str()) == cutFaceList[i]) {
                    updateCutFaceButtonState(i);
                    break;
                }
...
void PartWidget::updateDeformButton()
{
    ...
    if (part->deformAdjusted() || part->deformUnified)

...
void SceneOpenGLProgram::addShaderFromResource(QOpenGLShader::ShaderType type, const char* resourceName)
{
...
bool SceneOpenGLProgram::isCoreProfile() const
{
    return m_isCoreProfile;
}

void SceneOpenGLProgram::load(bool isCoreProfile)
{
    if (m_isLoaded)
        return;
    ...
    if (m_isCoreProfile) {
        ...
        addShaderFromResource(QOpenGLShader::Fragment, ":/shaders/scene_model_core.frag");
    } else {
        addShaderFromResource(QOpenGLShader::Vertex, ":/shaders/world_model.vert");
        addShaderFromResource(QOpenGLShader::Fragment, ":/shaders/scene_model.frag");
    }
...
void SceneOpenGLProgram::activeAndBindTexture(int location, QOpenGLTexture* texture)
{
...
void SceneOpenGLProgram::updateTextureImage(std::unique_ptr<QImage> image)
{
...
void SceneOpenGLProgram::updateNormalMapImage(std::unique_ptr<QImage> image)
{
    QMutexLocker lock(&m_imageMutex);
...
void SceneOpenGLProgram::updateMetalnessRoughnessAmbientOcclusionMapImage(std::unique_ptr<QImage> image,
...
void SceneOpenGLProgram::bindMaps(GLuint shadowDepthTexture)
{
...
void SceneOpenGLProgram::releaseMaps()
{
    if (m_texture)
        m_texture->release();
    if (m_normalMap)
...
int SceneOpenGLProgram::getUniformLocationByName(const std::string& name)
{

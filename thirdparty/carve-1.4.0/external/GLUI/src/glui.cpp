...
int GLUI::init( const char *text, long flags, int x, int y, int parent_window, int w, int h ) 
{
  int old_glut_window;
  ...
  if ( (flags & GLUI_SUBWINDOW) != GLUI_SUBWINDOW ) {  /* not a subwindow, creating a new top-level window */
    ...
    if (w > 0 && h > 0) {
      create_standalone_window( window_name.c_str(), x, y, w, h );
    } else {
...
void GLUI_Main::create_standalone_window( const char *name, int x, int y, int w, int h )
{
...
void glui_motion_func(int x, int y)
{
  ...
  if ( glut_window ) { /**  Was event in a GLUT window?  **/
    ...
    if (glut_window->glut_motion_CB)
      glut_window->glut_motion_CB( x, y );
    finish_drawing();
  } else {               
...
GLUI *GLUI_Master_Object::create_glui( const char *name, long flags,int x,int y, int w, int h )
{
  GLUI *new_glui = new GLUI;
  new_glui->init( name, flags, x, y, -1, w, h );
...
/*********************** GLUI_Master_Object::set_glutMotionFunc() **********/

void GLUI_Master_Object::set_glutMotionFunc(void (*f)(int x, int y))
{
  glutMotionFunc( glui_motion_func );
  add_cb_to_glut_window( glutGetWindow(), GLUI_GLUT_MOTION, (void*) f);
}

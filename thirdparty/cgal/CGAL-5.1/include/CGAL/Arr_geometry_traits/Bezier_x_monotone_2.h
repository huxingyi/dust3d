...
//
// $URL: https://github.com/CGAL/cgal/blob/v5.1/Arrangement_on_surface_2/include/CGAL/Arr_geometry_traits/Bezier_x_monotone_2.h $
// $Id: Bezier_x_monotone_2.h 3dc2804 2020-08-11T13:23:08+02:00 Ahmed Essam
                 ...
                 Bezier_cache& cache) const
{
  ...
  const std::pair<Algebraic, Algebraic>& range = _t_range (cache);
  const Algebraic& t_src = range.first;
  const Algebraic& t_trg = range.second;
                                 ...
                                 std::list<Point_2>& inter_pts) const
{
  ...
  bound_tr.compute_intersection_points(cp1, cp2, std::back_inserter(ipt_bounds));

  // Construct the approximated points.
  for (auto iter = ipt_bounds.begin(); iter != ipt_bounds.end(); ++iter) {
    ...
    if (! bound1.can_refine || ! bound2.can_refine) return false;
           ...
           Self& ovlp_cv) const
{
  ...
  if (self_intersect) {
    if (_xid == cv._xid) return false;
    if (_curve.has_no_self_intersections()) return false;
  }
  ...
  if (map_iter != inter_map.end()) {
    ...
    if (app_ok && ! self_intersect) inter_map[curve_pair] = inter_pts;
  }
  ...
  if (app_ok) {
    ...
    // if that it can fail in the following cases: (i) there are two
    // intersection points lying very close together, (ii) there exists an
    // intersection point whose multiplicity is greater than 1, or (iii) the
    // curves overlap. If the approximation went OK, then we know that we have
    // a simple intersection point (with multiplicity 1) if intersection point
    // is not rational (otherwise it is unknown: at this point, an intersection
    // point is rational if it was found as a control point during the de
    ...
    for (pit = inter_pts.begin(); pit != inter_pts.end(); ++pit) {
      ...
      if (in_range1 && in_range2) {
        ...
        ipts.push_back(Intersection_point(*pit, pit->is_rational() ? 0 : 1));
      }
    }
  ...
  if (do_ovlp) {
    ...
    if (left().equals (cv.left(), cache)) {
      ...
      if (_is_in_range(cv.right(), cache, t_right, is_endpoint)) {
        ...
        ovlp_cv = *this;
        ovlp_cv.right().add_originator(Originator(cv._curve, cv._xid, t_right));
      ...
      // endpoint.
      ipts.push_back(Intersection_point(left(), 0));
      return (false);
    }
      ...
      if (_is_in_range (cv.left(), cache, t_left, is_endpoint)) {
        ...
        ovlp_cv = *this;
        ovlp_cv.left().add_originator(Originator(cv._curve, cv._xid, t_left));
      ...
      // endpoint.
      ipts.push_back(Intersection_point(right(), 0));
      return false;
    }
  ...
  // of both curves.
  for (auto iit = inter_list.begin(); iit != inter_list.end(); ++iit) {
    if (_is_in_range (iit->s, cache) && cv._is_in_range(iit->t, cache)) {
      ...
      ipts.push_back(Intersection_point(pt, 0));
    }
  }
